...
server.patch("/UpdateUser", async (req, res) => {
  ...
  try {
    ...
    // 2️⃣ Define allowed fields that user can update
    ...
    // 3️⃣ Check if all keys inside Data are allowed
    const isUpdateAllowed = Object.keys(Data).every((key) => {
      return Allowed_Updates.includes(key);
    });

    // 4️⃣ If user tries to update restricted field → throw error
    if (!isUpdateAllowed) {
      return res.status(400).send("Update key not allowed.");
    }

    // 5️⃣ Update user by ID
    const updatedUser = await User.findByIdAndUpdate(
      UserId,   // Directly pass ID (correct way)
